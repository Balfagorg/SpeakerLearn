...
public:
    ...
    bool update_speaker_system_name(const std::string& id, const std::string& name);
    std::vector<SpeakerSystem> get_speaker_systems(const std::string& user_id);
    bool delete_speaker_system(const std::string& id);
    ...
    bool replace_calibration_results(const std::string& speaker_system_id,
                                     const std::vector<CalibrationResult>& results);
    std::vector<CalibrationResult> get_calibration_results(const std::string& speaker_system_id);

    // Active speaker
    bool set_active_speaker(const std::string& user_id, const std::string& speaker_system_id);
    std::string get_active_speaker(const std::string& user_id);
    ...
    std::optional<SourceProfile> get_source_profile(const std::string& platform_name);
