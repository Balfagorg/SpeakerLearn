...
bool DatabaseManager::update_speaker_system_name(const std::string& id, const std::string& name) {
    std::lock_guard<std::mutex> lock(db_mutex_);
    const char* sql = "UPDATE speaker_systems SET name = ? WHERE id = ?;";
    sqlite3_stmt* stmt = nullptr;
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) != SQLITE_OK) return false;
...
bool DatabaseManager::delete_speaker_system(const std::string& id) {
    std::lock_guard<std::mutex> lock(db_mutex_);
    const char* del_cap = "DELETE FROM speaker_capabilities WHERE speaker_system_id = ?;";
    sqlite3_stmt* stmt = nullptr;
    if (sqlite3_prepare_v2(db_, del_cap, -1, &stmt, nullptr) != SQLITE_OK) return false;
    sqlite3_bind_text(stmt, 1, id.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);

    const char* del_cal = "DELETE FROM calibration_results WHERE speaker_system_id = ?;";
    if (sqlite3_prepare_v2(db_, del_cal, -1, &stmt, nullptr) != SQLITE_OK) return false;
    sqlite3_bind_text(stmt, 1, id.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);

    const char* del_sys = "DELETE FROM speaker_systems WHERE id = ?;";
    if (sqlite3_prepare_v2(db_, del_sys, -1, &stmt, nullptr) != SQLITE_OK) return false;
    sqlite3_bind_text(stmt, 1, id.c_str(), -1, SQLITE_TRANSIENT);
    bool success = (sqlite3_step(stmt) == SQLITE_DONE);
    sqlite3_finalize(stmt);
    return success;
...
bool DatabaseManager::replace_calibration_results(const std::string& speaker_system_id,
                                                 const std::vector<CalibrationResult>& results) {
    std::lock_guard<std::mutex> lock(db_mutex_);
    const char* del_sql = "DELETE FROM calibration_results WHERE speaker_system_id = ?;";
    sqlite3_stmt* stmt = nullptr;
    if (sqlite3_prepare_v2(db_, del_sql, -1, &stmt, nullptr) != SQLITE_OK) return false;
    sqlite3_bind_text(stmt, 1, speaker_system_id.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);
    ...
    for (const auto& r : results) {
        ...
        sqlite3_bind_text(stmt, 2, r.speaker_system_id.c_str(), -1, SQLITE_TRANSIENT);
...
std::vector<CalibrationResult> DatabaseManager::get_calibration_results(const std::string& speaker_system_id) {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        cr.id = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 0));
        cr.speaker_system_id = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 1));
...
std::string DatabaseManager::get_active_speaker(const std::string& user_id) {
    std::lock_guard<std::mutex> lock(db_mutex_);
    const char* sql = "SELECT id FROM speaker_systems WHERE user_id = ? AND is_active = 1 LIMIT 1;";
    sqlite3_stmt* stmt = nullptr;
...
std::optional<SourceProfile> DatabaseManager::get_source_profile(const std::string& platform_name) {
    std::lock_guard<std::mutex> lock(db_mutex_);
    std::string key = platform_name;
    if (key == "local") key = "local_flac";
    if (key == "apple music") key = "apple_music";
    const char* sql = "SELECT id, platform_name, sub_bass_offset, bass_offset, low_mid_offset, mid_offset, high_offset, air_offset, description FROM source_profiles WHERE platform_name = ?;";
    sqlite3_stmt* stmt = nullptr;
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) != SQLITE_OK) return std::nullopt;
    sqlite3_bind_text(stmt, 1, key.c_str(), -1, SQLITE_TRANSIENT);

    std::optional<SourceProfile> result;
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        SourceProfile p;
        p.id = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 0));
        p.platform_name = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 1));
        p.sub_bass_offset = sqlite3_column_double(stmt, 2);
        p.bass_offset = sqlite3_column_double(stmt, 3);
        p.low_mid_offset = sqlite3_column_double(stmt, 4);
        p.mid_offset = sqlite3_column_double(stmt, 5);
        p.high_offset = sqlite3_column_double(stmt, 6);
        p.air_offset = sqlite3_column_double(stmt, 7);
